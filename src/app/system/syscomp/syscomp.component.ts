...
export class SysCompComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle("Company - Green Field");
      this.refreshMe();
  }
  ...
  openEntry = function (result: SysCompModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(SysCompEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
