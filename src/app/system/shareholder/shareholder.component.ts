...
export class ShareholderComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle("Shareholder - Green Field");
      this.refreshMe();
  }
  ...
  openEntry = function (result: ShareholderModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(ShareholderEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
