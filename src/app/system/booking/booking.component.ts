...
export class BookingComponent implements OnInit {
    ...
    displayedColumns: string[] =
        ['select','bookingNo','freightAmt','freightAmtSDG','clearance','docCost','clearDes','clearDesSDG', 'edit'];
...
;
  ...
  ngOnInit() {
    this.titleService.setTitle("Booking - Green Field");
      this.refreshMe();
  }
  ...
  refreshMe() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.freightAmt = "Freight(USD)"
      this.freightAmtSDG = "Freight(SDG)"
      this.clearance = "Clearance"
      this.docCost = "Document"
      this.clearDes = "DBX Clearance(AED)"
      this.clearDesSDG = "DBX Clearance(SDG)"
      ...
      this.bookingNo = "حجز"
      this.freightAmt = "شحن(دولار)"
      this.freightAmtSDG = "شحن(جنيه)"
      this.clearance = "التخليص"
      this.docCost = "التوثيق"
      this.clearDes = "(دولار) DBX تخليص"
      this.clearDesSDG = "(جنيه) DBX تخليص"
  ...
  open = function(result:Send){
    if (result === undefined) {
      this.dialogRef = this.dialog.open(CompanyBankBranchEntryComponent2, {
        disableClose: true,
  ...
  openEntry = function (result: BookingModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(BookingEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
